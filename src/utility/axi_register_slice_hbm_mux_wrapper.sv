// *************************************************************************
//
// Thin wrapper around `axi_register_slice_hbm_mux` — a 256b AXI3 register
// slice with a 4-bit ID, used between the RM's `m_axi_hbm_31` partition
// output (the muxed XDMA+RM landing on HBM port 31) and the
// `decouple_axi_hbm_31` decoupler in alveo_host_top.
//
// Same width-translation rules as `axi_register_slice_hbm_wrapper`:
//   * AWLEN / ARLEN: axi_if LEN_W=4 matches AXI3 directly.
//   * AWLOCK / ARLOCK: AXI3 is 2-bit; project's axi_if carries 1-bit
//     lock (AXI4 convention).  Zero-extend on input, truncate on output.
//   * AWREGION / ARREGION: AXI3 has no REGION; tied to 0 on the master
//     side, dropped on the slave side.
//
// *************************************************************************
`timescale 1ns/1ps
module axi_register_slice_hbm_mux_wrapper (
  input         aclk,
  input         aresetn,
  ...
  axi_register_slice_hbm_mux u_rs (
    .aclk          (aclk),
    .aresetn       (aresetn),
