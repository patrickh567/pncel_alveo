...
`endif
  ...
  // axi_protocol_converter_0 + axi_hbm_switch.s01 chain before
  // reaching HBM port 31.
...
`endif
  ...
  // =========================================================================
  // AXI Full Switch (1 slave -> 2 masters) for the host DMA path.
  //
  // XDMA's `axi_dma` is now narrowed to 256 b BEFORE the switch (by
  // `axi_dwidth_converter_0_inst` below); the switch itself is
  // reconfigured to take a 256-bit slave port.  M00 (HBM-bound) is
  // 256 b; M01 (HBICAP) stays 32 b — the switch handles the 256→32
  // narrow on the HBICAP branch internally.
  //
  // Host-DMA address map (see src/utility/vivado_ip/axi_dma_switch.tcl):
  //   0x0000_0000_0000_0000 - 0x0000_0001_FFFF_FFFF   M00 → HBM (8 GB,  256 b)
  ...
  // 512 → 256 dwidth on XDMA's bulk DMA bus, AXI4 both sides.  Sits
  // directly between `axi_dma` and the dma switch (no other IP
  // upstream of the dwidth converter).
  ...
  // axi_hbm_switch multiplexes two AXI4/256 slaves down onto HBM
  // port 31 (the muxed landing — direct RM port 31 is 4-bit ID and
  // arrives on s01; XDMA-direct path comes in on s00).  Both slaves
  // share AXI4/256 (NUM_SI > 1 forces a uniform shape); the switch
  // performs only an AXI4 → AXI3 protocol conversion on m00, no
  // width step inside.
  //
  //   s00  axi_dma_hbm                                   — AXI4/256
  //                                                       (host DMA, after
  //                                                        the 512→256
  //                                                        dwidth that
  //                                                        sits in front
  //                                                        of axi_dma_switch)
  //   s01  s_axi_hbm_mux_axi4_256                        — AXI4/256
  //                                                       (RM port 31 after
  //                                                        AXI3→AXI4 protocol)
  //   m00  axi_hbm_mux                                   — AXI3/256
  //                                                       (HBM port 31 input)

  // RM leg (s_axi_hbm_31): AXI3/256 → AXI4/256 (protocol).
  axi_if #(.ADDR_W(64), .DATA_W(256), .ID_W(4))
  ...
  hbm_wrapper hbm_inst (
    ...
    // with the RM's s_axi_hbm_31 leg (s01 after the protocol converter).
