...
module alveo_host_top #(
  ...
  // Boundary-side register slice on the mux path.  Sits between the
  // partition output (`dynamic_inst.m_axi_hbm_31` → `axi_hbm_31_dy`)
  // and the decoupler's `decouple_axi_hbm_31` slave input — same role
  // as the `hbm_slice_bank_*` slices but on the partition-pad side of
  // the decoupler, since the mux's static-side converter chain leaves
  // its post-decoupler timing path long.  4-bit ID matches the mux.
  ...
  // the decoupler and `alveo_u50_static.s_axi_hbm_NN` to break the
  // long timing path into the HBM IP.  See `hbm_slice_bank_a/_b`.
  // Port 31 (the mux landing) is intentionally unsliced — its
  // static-side protocol_converter chain already provides multi-stage
  // buffering before the HBM switch.
  ...
  // IP.  Splits the long timing path from the partition boundary into
  // the HBM IP, and gives the floorplan a register cell to anchor at
  // the decoupler's output.  Mux path is unsliced (already buffered).
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  alveo_u50_dynamic dynamic_inst (
